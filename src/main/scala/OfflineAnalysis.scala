...
def main(args:Array[String]){
  ...
  val HRQps = hiveCtx.sql("SELECT FIRST(time),ps,count(*) as count FROM hrq.original WHERE time> "+HRQlast+" GROUP BY ps having count > 100 ORDER BY count DESC LIMIT 50").rdd.map(p=>Row(timestamp,p(1).toStrin(),p(2).toString().toInt))
  val HRQipds = hiveCtx.sql("SELECT ipd,cast(sum(TTL) as bigint) as sum FROM hrq.original WHERE time> "+HRQlast+" GROUP BY ipd ORDER BY sum DESC LIMIT 50").rdd.map(p=>Row(timestamp,p(0).toString(),p(1).toString().toInt))
  val HRQipss = hiveCtx.sql("SELECT ips,cast(sum(TTL) as bigint) as sum FROM hrq.original WHERE time> "+HRQlast+" GROUP BY ips ORDER BY sum DESC LIMIT 50").rdd.map(p=>Row(timestamp,p(0).toString(),p(1).toString().toInt))
  val HRQcount = hiveCtx.sql("SELECT count(*) FROM hrq.original WHERE time> "+HRQlast).rdd.map(p=>Row(timestamp,p(0).toString().toInt))
  val HRQsum = hiveCtx.sql("SELECT cast(sum(TTL) as bigint) FROM hrq.original WHERE time> "+HRQlast).rdd.map(p=>Row(timestamp,p(0).toString().toInt))
  ...
  val HRSipds = hiveCtx.sql("SELECT ipd,cast(sum(TTL) as bigint) as sum FROM hrs.original WHERE time> "+HRSlast+" GROUP BY ipd ORDER BY sum DESC LIMIT 50").rdd.map(p=>Row(timestamp,p(0).toString(),p(1).toString().toInt))
  val HRSipss = hiveCtx.sql("SELECT ips,cast(sum(TTL) as bigint) as sum FROM hrs.original WHERE time> "+HRSlast+" GROUP BY ips ORDER BY sum DESC LIMIT 50").rdd.map(p=>Row(timestamp,p(0).toString(),p(1).toString().toInt))
  val HRScount = hiveCtx.sql("SELECT count(*) FROM hrs.original WHERE time> "+HRSlast).rdd.map(p=>Row(timestamp,p(0).toString().toInt))
  val HRSsum = hiveCtx.sql("SELECT cast(sum(TTL) as bigint) FROM hrs.original WHERE time> "+HRSlast).rdd.map(p=>Row(timestamp,p(0).toString().toInt))
  ...
  val DRQcount = hiveCtx.sql("SELECT count(*) FROM drq.original WHERE time> "+DRQlast).rdd.map(p=>Row(timestamp,p(0).toString().toInt))
  ...
  val DRScount = hiveCtx.sql("SELECT count(*) FROM drs.original WHERE time> "+DRSlast).rdd.map(p=>Row(timestamp,p(0).toString().toInt))
  ...
  val allschema = StructType(List(StructField("time",IntegerType,true),StructField("count",IntegerType,true)))
  val sizeschema = StructType(List(StructField("time",IntegerType,true),StructField("IP",StringType,true),StructField("size",IntegerType,true)))
  ...
  sqlContext.createDataFrame(HRQipss,sizeschema).write.mode("append").jdbc("jdbc:mysql://172.16.0.104:3306/DBNS", "DBNS.HRQipss", prop)
  sqlContext.createDataFrame(HRQipds,sizeschema).write.mode("append").jdbc("jdbc:mysql://172.16.0.104:3306/DBNS", "DBNS.HRQipds", prop)
  sqlContext.createDataFrame(HRQcount,allschema).write.mode("append").jdbc("jdbc:mysql://172.16.0.104:3306/DBNS", "DBNS.HRQcount", prop)
  sqlContext.createDataFrame(HRQsum,allschema).write.mode("append").jdbc("jdbc:mysql://172.16.0.104:3306/DBNS", "DBNS.HRQsum", prop)
  ...
  sqlContext.createDataFrame(HRSipss,sizeschema).write.mode("append").jdbc("jdbc:mysql://172.16.0.104:3306/DBNS", "DBNS.HRSipss", prop)
  sqlContext.createDataFrame(HRSipds,sizeschema).write.mode("append").jdbc("jdbc:mysql://172.16.0.104:3306/DBNS", "DBNS.HRSipds", prop)
  sqlContext.createDataFrame(HRScount,allschema).write.mode("append").jdbc("jdbc:mysql://172.16.0.104:3306/DBNS", "DBNS.HRScount", prop)
  sqlContext.createDataFrame(HRSsum,allschema).write.mode("append").jdbc("jdbc:mysql://172.16.0.104:3306/DBNS", "DBNS.HRSsum", prop)
  ...
  sqlContext.createDataFrame(DRQcount,allschema).write.mode("append").jdbc("jdbc:mysql://172.16.0.104:3306/DBNS", "DBNS.DRQcount", prop)
  ...
  sqlContext.createDataFrame(DRScount,allschema).write.mode("append").jdbc("jdbc:mysql://172.16.0.104:3306/DBNS", "DBNS.DRScount", prop)
