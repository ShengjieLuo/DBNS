...
def main(args:Array[String]){
  ...
  val HRQtype = hiveCtx.sql("SELECT count(*),type FROM hrq.original GROUP BY type WHERE time> "+HRQlast).rdd.map(p=>Row(timestamp,p(0).toString(),p(1).toString().toLong))
  val HRQtypes = hiveCtx.sql("SELECT cast(sum(TTL) as bigint),type FROM hrq.original GROUP BY type WHERE time> "+HRQlast).rdd.map(p=>Row(timestamp,p(0).toString(),p(1).toString().toLong))
