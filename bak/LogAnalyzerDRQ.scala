...
def main(args:Array[String]){
  ...
  lineMap.saveAsTextFiles("file:///usr/local/spark/mycode/DBNS/backup/drq.txt")
  ...
  val schema = StructType(List(StructField("time", StringType, true),StructField("ips", StringType, true),StructField("ipd", StringType, true),StructField("name", StringType, true),StructField("type", StringType, true),StructField("class", StringType, true)))
  lines.foreachRDD(rdd =>
  {
    val rowrdd = hiveCtx.createDataFrame(rdd.map(p => Row(p(0).trim, p(1).trim, p(2).trim, p(3).trim,p(4).trim,p(5).trim)), schema)
